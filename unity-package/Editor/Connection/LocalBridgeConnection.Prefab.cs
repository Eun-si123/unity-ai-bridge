...
namespace UnityAiBridge.Editor.Connection
{
    internal static partial class LocalBridgeConnection
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                await HandlePrefabPropertyApplyAsync(current, command, rawJson, cancellationToken);
                return true;
            }
        ...
        private static async Task HandlePrefabPropertyApplyAsync(
            ClientWebSocket current,
            BridgeCommandDto command,
            string rawJson,
            CancellationToken cancellationToken)
        {
            ...
            {
                await SendErrorAsync(current, command.requestId, "validation", "risk_mismatch",
                    "prefab.property.apply requires risk='destructive' because it persists a Prefab Asset property override to disk without a bridge rollback guarantee.", cancellationToken);
                return;
            }
            ...
            try
            {
                ...
                var response = new BridgePrefabPropertyApplyResultDto
                {
                    ...
                    warnings = result.replayed
                        ? Array.Empty<string>()
                        : new[] { "Applying a Prefab property override is a persistent Prefab Asset disk write; the bridge does not advertise Unity Undo for this asset change." },
                    changedTargets = Array.Empty<BridgeChangedTargetDto>(),
                    dirtyState = result.sceneIsDirtyAfter ? "dirty" : "unchanged",
                    undo = new BridgeUndoDto { available = false, groupName = string.Empty },
                    compileState = "idle",
                };
                await SendJsonAsync(current, JsonUtility.ToJson(response), cancellationToken);
            ...
            {
                await SendErrorAsync(current, command.requestId, "validation", "prefab_property_unsupported", exception.Message, cancellationToken);
            }
            ...
            {
                await SendErrorAsync(current, command.requestId, "unity_api", "prefab_property_apply_verification_failed", exception.Message, cancellationToken);
            }
            catch (Exception exception)
            {
        ...
        [Serializable]
        private sealed class PrefabPropertyApplyBridgeCommandDto { public PrefabPropertyApplyCommandArgumentsDto arguments; }
        [Serializable]
