...
- `prefab.inspect` — inspect one exact Prefab Asset with bounded hierarchy/component metadata and its Unity dependency hash,
- `prefab.instantiate` — instantiate one exact Prefab Asset into the active scene while preserving Prefab linkage, with scene+asset preconditions, Undo, native verification, and replay protection,
...
`prefab.inspect` accepts an exact Prefab Asset path plus `maxDepth=0..32` and `maxNodes=1..500`. It loads the Prefab Asset through Unity `AssetDatabase`, verifies that the path refers to a Prefab Asset rather than a folder/arbitrary asset, and returns its GUID, `GetAssetDependencyHash`, Prefab asset type, root name, bounded preorder hierarchy, component type names, and explicit truncation metadata. It does not instantiate the Prefab or change the scene.

`prefab.instantiate` accepts:

- `prefabPath` — exact project-relative Prefab Asset path,
- `expectedPrefabDependencyHash` — exact dependency hash from recent inspection of that Prefab,
- `mutationId` — same-session retry identity; generated by the Node bridge when omitted,
- `expectedStateEpoch` and `expectedStateRevision` — required fresh active-scene precondition.

Scene and Prefab state are deliberately separate concurrency domains. The scene token protects where the instance is being created; the Prefab dependency hash protects what asset is being instantiated. Unity rechecks the Prefab hash immediately before native creation and during semantic verification, instantiates with `PrefabUtility.InstantiatePrefab`, registers the created hierarchy with Undo, marks the scene dirty, and resolves the new root through `GlobalObjectId`. Native verification checks the active-scene target plus `GetPrefabAssetPathOfNearestInstanceRoot` and `GetCorrespondingObjectFromSource` linkage back to the requested Prefab Asset. If verification fails after creation, the common transaction rolls the Undo group back and verifies the created root is absent. A same-id replay succeeds only while the same linked instance still exists; Undo/deletion/unlinkage produces `stale_target/mutation_replay_stale` and never silently creates another copy. See [`PREFAB_WORKFLOWS.md`](PREFAB_WORKFLOWS.md).
...
Successful `asset.search`, `asset.inspect`, and `prefab.inspect` results are read-only: they report `dirtyState: "unchanged"` and do not create Undo metadata. Search/dependency/hierarchy truncation remains explicit operation-level result metadata.

A first-time successful `prefab.instantiate` reports the linked root as changed, the active scene as dirty, and an available Undo group. A same-id replay reports `replayed=true`, no new changed target, and no new Undo mutation because no second instance is created.
