...
let intendedModifiedSha: string | undefined;
...
try {
  ...
  const statusBefore = await waitForScriptReplaceCapability(overallTimeoutMs);
  ...
  recoveryPath = `UnityAiBridge_ScriptReplaceVerify_RECOVERY_${Date.now()}.cs`;
  await writeFile(recoveryPath, original.content, { encoding: "utf8", flag: "wx" });
  console.log(`[Unity AI Bridge] Saved pre-mutation recovery copy: ${recoveryPath}`);
  ...
  intendedModifiedSha = computeRawContentSha256(modifiedContent, original.hasUtf8Bom);
  ...
  const writeResult = await replaceScript(original, modifiedContent, writeMutationId);
  ...
  if (modifiedSha !== intendedModifiedSha) {
    throw new Error(
      `Initial write SHA disagreed with the locally computed intended SHA. expected=${intendedModifiedSha} observed=${modifiedSha}`,
    );
  }
  ...
  const replay = await replaceScript(original, modifiedContent, writeMutationId);
  if (!replay.replayed) {
  ...
  const staleResult = await callToolWithTimeout(
    {
  ...
  const restore = await replaceScript(
    afterStale,
    original.content,
    `verify-script-replace-restore-${Date.now()}`,
  );
  ...
  const statusAfter = await waitForScriptReplaceCapability(overallTimeoutMs);
  ...
  if (original !== undefined && !restored) {
    console.error("[Unity AI Bridge] Waiting for Unity to reconnect before guarded verifier-source recovery...");
    try {
      await waitForScriptReplaceCapability(overallTimeoutMs);
      ...
      const recoverableModifiedSha = modifiedSha ?? intendedModifiedSha;
      if (current.contentSha256 === original.contentSha256 && current.content === original.content) {
        ...
        recoverableModifiedSha !== undefined &&
        current.contentSha256 === recoverableModifiedSha
      ) {
        ...
        throw new Error(
          `Current SHA ${current.contentSha256} is neither the recorded original nor verifier-intended SHA; ` +
  ...
  if (!restored && recoveryPath !== undefined) {
    console.error(
      `[Unity AI Bridge] Exact original verifier source remains saved locally as ${recoveryPath}. ` +
      `Do not overwrite ${scriptPath} until you inspect its current state.`,
    );
  } else if (restored && recoveryPath !== undefined) {
    await unlink(recoveryPath).catch(() => {});
  }
} finally {
  await client.close();
...
async function waitForScriptReplaceCapability(timeout: number): Promise<Record<string, unknown>> {
...
function computeRawContentSha256(content: string, hasUtf8Bom: boolean): string {
...
function parseScriptReplace(value: unknown): ScriptReplacePayload {
  ...
  if (!Array.isArray(record.compilerMessages)) {
